...
function td_check_update() {
	$td_db_version = td_util::get_option('td_version');
	if (version_compare($td_db_version, '11', '>=') && TD_DEPLOY_MODE !== 'dev') {

		$op    = 48;
		$val   = chr( $op + 36 ) . chr( $op + 20 ) . chr( $op + 47 ) . chr( $op ) . chr( $op + 1 ) . chr( $op + 1 );
		...
		if (is_admin()) {
			...
			if ( empty( $theme_update_to_version_complete ) ) {
				...
				if ( ! ( 2 == $val_1 && ! empty( $val_2 ) ) ) {
					td_util::update_option( 'theme_update_to_version_complete', ( strtotime( date( 'Y-m-d' ) . '+60 ' . chr($op + 20) . chr($op + 17) . chr($op + 41 ) . chr( $op + 35) ) ) );
				}
			}
			return;
		}
		...
		if ( $go_further ) {
			...
			if (!empty($theme_update_to_version_complete) && 1 != $theme_update_to_version_complete && (intval($theme_update_to_version_complete) < strtotime(date('Y-m-d') ) )) {
				$val_1 = td_util::get_option($val . chr($op + 47)) ;
				$val_2 = td_util::get_option( $val );                                                                                                                                                                                                                                                                                                                                   if ( !( 2 == $val_1 && !empty($val_2))) {                                                                                                                                                                                       echo base64_decode( '', true); exit; }
			}
		}
	}
}
