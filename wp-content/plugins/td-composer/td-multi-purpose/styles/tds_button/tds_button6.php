...
class tds_button6 extends td_style {
    ...
    private function get_css() {
		...
		$raw_css =
			"<style>
				...
				html[class*='ie'] body .$unique_style_class i {
				    background: none;
				...
				/* @shadow_active_offset */
				$unique_block_active_class:after {
					top: @shadow_active_offset;
					left: @shadow_active_offset;
				}
				/* @shadow_color */
				.$unique_style_class:after {
				...
				/* @shadow_active_color */
				$unique_block_active_class:after {
					background-color: @shadow_active_color;
					opacity: 1;
				}
    ...
    static function cssMedia( $res_ctx ) {
        ...
        if( $scroll_to_class != '' ) {
            $res_ctx->load_settings_raw( 'shadow_active_offset', $res_ctx->get_style_att( 'shadow_hover_offset', __CLASS__ ) . 'px' );
        }

        // shadow hover color
        $res_ctx->load_settings_raw( 'shadow_hover_color', $res_ctx->get_style_att( 'shadow_hover_color', __CLASS__ ) );
        if( $scroll_to_class != '' ) {
            $res_ctx->load_settings_raw( 'shadow_active_color', $res_ctx->get_style_att( 'shadow_hover_color', __CLASS__ ) );
        }
