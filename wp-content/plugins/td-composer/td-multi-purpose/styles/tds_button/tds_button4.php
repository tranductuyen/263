...
class tds_button4 extends td_style {
    ...
    static function cssMedia( $res_ctx ) {
        ...
        if( $scroll_to_class != '' ) {
            $res_ctx->load_settings_raw( 'background_active_color', $res_ctx->get_style_att( 'background_hover_color', __CLASS__ ) );
        }
    ...
    function render( $index_style = '' ) {
        ...
        $buffy .= '<div class="' . self::get_group_style( __CLASS__ ) . ' td-fix-index">';
            $buffy .=  '<div class="' . self::get_class_style(__CLASS__) . ' ' . $this->get_shortcode_att('button_size', $this->index_style) . '-wrap ' . $this->unique_style_class . '">';
                $buffy .= '<a href="' . $button_url . '" class="tdm-btn tdm-button-a ' . $this->get_shortcode_att('button_size', $this->index_style) . '" ' . $td_link_rel . $target . $data_ga_event_cat . $data_ga_event_action . $data_ga_event_label . $data_fb_event_name . $data_fb_event_cotent_name . '>';
