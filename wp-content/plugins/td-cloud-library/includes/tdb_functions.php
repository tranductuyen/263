	...
	if ( is_user_logged_in() &&  current_user_can( 'switch_themes' ) && is_admin_bar_showing() ) {
	    ...
	    if ( tdb_state_content::has_wp_query() ) {
            ...
            if ( 'single' === tdb_state_template::get_template_type() ) {
                ...
                td_show_woo_product_category_template_menu();
            ...
            if ( 'woo_product' === tdb_state_template::get_template_type() && (function_exists('is_product') && is_product()) ) {
                $class_menu_item = 'woo_product';

            } else if ( 'woo_archive' === tdb_state_template::get_template_type() || (function_exists('is_product_category') && is_product_category()) ) {
            ...
            ( function_exists('is_product_category') && is_product_category() && ! is_admin() ) ||
...
function td_show_woo_product_category_template_menu() {
    global $wp_admin_bar;
    ...
    if ( $queried_object instanceof WP_Term ) {
        ...
        if ( empty( $tdb_woo_archive_template ) ) {
            ...
            if (empty($default_template_id)) {
                $current_template .= ' DEFAULT';
            } else if ( td_global::is_tdb_template( $default_template_id, true ) ) {
                $current_template .= ' ' . get_the_title( td_global::tdb_get_template_id( $default_template_id ) );
            }
...
if ( ! empty( $tdbTemplateType ) || ( ! empty( $post_type ) && 'tdb_templates' === $post_type ) || ( isset( $_GET['page'] ) && ( $_GET['page'] === 'td_theme_welcome' ) ) ) {
